...
###############################################################################################
    ...
    $userObjectID = Get-AzureRmaduser -UserPrincipalName $account | Select-Object -ExpandProperty Id 
    $userObjectID = $userObjectID.Guid |Out-String
    Write-Host "`t User Account: $account" -foregroundcolor Green;
